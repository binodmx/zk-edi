...
class EdgeServer:
    ...
    # TODO: implement malicious/faulty behavior
    def run(self):
    ...
    def run_cm(self):
        ...
        # when local verdict is received, take action
        # when global verdict is received, take action
    ...
    def run_ch(self):
        ...
        while True:
            ...
            if time.time() - self.t0 > self.dt1 or p_count == len(
                ...
                break
        sp_count = len(self.similar_proofs)
        if sp_count > len(self.clusters[self.c_id])/2:
        ...
        # Then if sp_count exceeds 50%, find majority.
        while True:
            ...
            if time.time() - self.t0 > self.dt1 + self.dt2 or p_count == len(
                self.cluster_heads):
                break
        # Aggregate proofs of similar cluster heads
        if self.similar_proof_count > self.n/2:
            ...
            for id, (pks, agg_proof) in self.distinct_agg_proofs.items():
                if agg_proof and PopSchemeMPL.fast_aggregate_verify(pks, hash_d, agg_proof):
                    ids.append(id)
            inter_agg_proof = PopSchemeMPL.aggregate([
        ...
        for s_id in self.clusters[self.c_id]:
            threading.Thread(target=self.edge_servers[s_id].set_global_verdict, 
                             args=(ids, inter_agg_proof,)).start()
    ...
    def set_local_verdict(self, ids, agg_proof):
        ...
        if agg_proof:
            pks = [self.edge_servers[id].public_key for id in ids]
            self.local_verdict = PopSchemeMPL.fast_aggregate_verify(pks, self.hash_d, agg_proof)
            self.logger.log(f"Local integrity reached in {(time.time()-self.t0)*1000}ms.")

    def set_global_verdict(self, ids, agg_proof):
        latency = self.latency_matrix[self.ch_id][self.id]
        if latency >= 0:
            time.sleep(latency)
        if agg_proof:
            pks = [self.edge_servers[id].public_key for id in ids]
            self.global_verdict = PopSchemeMPL.fast_aggregate_verify(pks, self.hash_d, agg_proof)
            self.logger.log(f"Global integrity reached in {(time.time()-self.t0)*1000}ms.")
